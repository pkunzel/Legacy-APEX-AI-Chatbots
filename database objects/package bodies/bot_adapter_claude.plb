 ...
 *        The package is called by bot_claude_provider_t, not directly by the facade.
 */
create or replace package body bot_adapter_claude as
   ...
   begin
      ...
      l_history := bot_agent_util.parse_message_array(
         ...
         p_package_name => 'bot_adapter_claude'
      );
   ...
   begin
      ...
      exception
         when others then
            apex_debug.message(
               'bot_adapter_claude.parse_response: Error parsing JSON response: '
               || sqlerrm
            );
    ...
    * @description Adapter algorithm entry point used by bot_claude_provider_t.
    */
   function get_text_response (
      p_url              in varchar2,
   ...
   begin
      ...
      apex_debug.message('bot_adapter_claude.get_text_response: Making API request...');
   ...
   exception
      when utl_http.request_failed then
         apex_debug.message('bot_adapter_claude.get_text_response: HTTP request failed.');
         return 'HTTP request failed.';
      when others then
         apex_debug.error(
            'Unexpected error in bot_adapter_claude.get_text_response: '
