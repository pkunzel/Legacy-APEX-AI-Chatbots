...
| `CB_CHATBOT_ARCHIVES` | `tables/cb_chatbot_archives.sql` | Stores one complete archived conversation per row as a JSON transcript, with chatbot ID, name, system-prompt, running-summary, and summary-prompt snapshots. |
...
| `CB_AGENT_UTIL` | `packages/cb_agent_util.sql` | Shared validation, structured system-context JSON, message helpers, and HTTP request helper. |
...
| `CB_AGENT_UTIL` | `package bodies/cb_agent_util.plb` | Implements structured system-context JSON construction/parsing/flattening, message handling, and HTTP POST behavior. |
| `CB_ADAPTER_OPENAI` | `package bodies/cb_adapter_openai.plb` | Flattens system context, builds OpenAI-compatible chat-completion payloads, and extracts assistant text. |
| `CB_ADAPTER_CLAUDE` | `package bodies/cb_adapter_claude.plb` | Builds Anthropic system blocks with stable-prefix/summary cache breakpoints, logs cache usage, and extracts assistant text. |
