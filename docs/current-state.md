...
- The application layer owns screen flow, page items, button behavior, message
  ...
  returns text.
- Vector embeddings are saved and used, but no vector index is required yet.
...
response to `CB_CHATBOTS.CURRENT_SUMMARY`, and sets `IS_SUMMARIZED = 'Y'` plus
`SUMMARIZED_DATE = SYSDATE` for the included rows.
...
| Message storage | Conversation `MESSAGE` is capped at `VARCHAR2(8000 CHAR)` for POC simplicity. |
